...
alzhimer_data = pd.read_csv("data/alzhimer.txt",delim_whitespace="",header=None)
alzhimer_node = alzhimer_data[0]
alzhimer_score = alzhimer_data[1]
...
fp1 = open("output_degree/alzhimer_degrees.txt","w")
...
for i in range(len(alzhimer_data)):
    degree = G.degree(alzhimer_node[i])
    fp1.write(str(alzhimer_node[i])+","+str(alzhimer_score[i])+","+str(degree)+"\n")
